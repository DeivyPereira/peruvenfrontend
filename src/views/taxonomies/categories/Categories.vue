                  ...
                  <v-card-title>
                      <v-layout row>
                          <v-flex lg-10 xs-10 md-10>
                            <span class="headline" v-if="isEdit == false">Nueva Categoría</span>
                            <span class="headline" v-if="isEdit">Editar Categoría</span>
                          </v-flex>
...
export default {
  ...
  methods: {
      ...
      save( id = null ){
          ...
          if( ! this.isEdit ){
              axios.get( 'category/check-item/' + data.name ).then( response => {
                if( response.data.status ){
                  ...
                  this.saveLoader = false
                  this.snackbar = { show: true, text: 'Esta Categoría ya existe', color: 'error',icon:'fa fa-warning' }
                }
              })
          } else {
              axios.put( 'category/' + this.id, data ).then( response => {
                  this.snackbar = { show: true, text: 'Categoría editada con éxito', color: 'success',icon:'fa fa-check' }
                  this.saveLoader = false
                  this.getList()
                  this.saveLoader = false
              }).catch( error => {
