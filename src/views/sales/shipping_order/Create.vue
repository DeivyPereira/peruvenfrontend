  ...
  <div id="createShippOrder">
          ...
          <v-stepper-content step="1">
            <v-layout wrap>
              <v-flex sm12 md12 lg12>
                ...
                <v-layout wrap>
                  <v-flex sm6 md4 lg4 v-for="items in basic.taxes" :key="items.id">
                    <v-checkbox
                      v-model="taxes"
              ...
              <v-flex sm12 md6 lg6 style="text-align: center">
                <v-btn color="info" @click="dialog.selectPackage = true">Asignar a fecha de salida</v-btn>
                <h4
                  v-if="form.package.package_id != ''"
                >Fecha de salida: {{ form.details.out_date }}</h4>
              </v-flex>
            </v-layout>
            <v-layout wrap>
          ...
          <v-stepper-content step="4">
            <v-form ref="addOrderForm">
              <v-layout wrap>
                ...
                <v-flex lg4 md4 sm12 xs12>
                  <v-select
                    label="Seleccione moneda del seguro"
                    :items="basic.money"
                    v-model="form.details.currency_ensurance"
                    @input="blurValidateOrder()"
                    item-text="money"
                    item-value="money"
                  ></v-select>
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    <tr v-if="form.weight.length > 0">
                      ...
                      <td style="background: rgb( 230,230,230 ); text-align: right">{{ form.total }}</td>
                      <td style="background: rgb( 230,230,230 );">{{ form.details.currency }}</td>
                    </tr>
                    <tr v-if="form.products.length > 0">
                      <td></td>
                      <td></td>
              ...
              <v-layout>
                <v-flex lg6 md6 sm12 xs12>
                  <v-btn flat @click="e1 = 3">Regresar</v-btn>

                  <v-btn color="primary" :disabled="disabled.orderform" @click="e1 = 5">Continuar</v-btn>
                </v-flex>
                <v-flex lg6 md6 sm12 xs12 style="text-align: right">
                  <v-btn color="success" @click="openAddProduct()" flat>
                    <i class="fa fa-plus"></i>&nbsp;
                    Agregar Producto
                  </v-btn>
                </v-flex>
                <v-flex lg6 md6 sm12 xs12 style="text-align: right">
                  <v-btn color="success" @click="openAddWeight()" flat>
                    <i class="fa fa-plus"></i>&nbsp;
                    Agregar Peso
                  </v-btn>
                </v-flex>
    ...
    <v-dialog v-model="dialog.searchClient" max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" @click.native="dialog.searchClient = false">Cerrar</v-btn>
          <v-btn
            color="success"
            :loading="saveLoader"
    ...
    <v-dialog v-model="dialog.searchAfiliate" scrollable max-width="600px">
      <v-card>
        <v-card-text>
          <div class="table-responsive">
            <table>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" @click.native="dialog.searchAfiliate = false">Cerrar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="dialog.addProduct" max-width="800px">
      <v-form ref="productForm">
        <v-card>
              ...
              <v-layout wrap>
                ...
                <v-flex lg12 md12 sm12 xs12>
                  <v-text-field
                    ...
                    :rules="[v => !!v || 'Campo requerido']"
                    v-model="productToInsert.description"
                  ></v-text-field>
                </v-flex>
                <v-flex lg6 md6 sm12 xs12>
                  <v-select
                ...
                <v-flex lg4 md4 sm12 xs12>
                  <v-text-field
                    ...
                    v-model="productToInsert.weight"
                  ></v-text-field>
                </v-flex>

                <v-flex lg4 md3 sm6 xs6>
                  <v-text-field
                    label="Precio del producto"
                    :rules="onlyNumbers"
                    @keyup="getTotalProductToInsertEnsurance()"
                    persistent-hint
                    hint="Solo valores numéricos"
                ...
                <v-flex lg4 md6 sm12 xs12>
                  <div style="margin: 0 auto; width: 90%; text-align: left">
                    <v-label>¿Item asegurado?</v-label>
                    <v-radio-group v-model="productToInsert.ensurance" row>
                      <v-radio
                ...
                <v-flex lg4 md6 sm12 xs12 v-if="productToInsert.ensurance == 1">
                  <v-text-field
                ...
                <v-flex lg12 md12 sm12 xs12 v-if="productToInsert.ensurance == 1">
                  <v-text-field
                    label="Total del Seguro"
                    disabled
                    v-model="productToInsert.price_ensurance"
                  ></v-text-field>
                </v-flex>
                ...
                <v-flex lg12 md12 sm12 xs12 v-if="productToInsert.ensurance == 1">
                  ...
                  >Moneda Seguro: {{ form.details.currency_ensurance }}</h5>
                </v-flex>
              </v-layout>
            </v-container>
          </v-card-title>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="error" @click.native="dialog.addProduct = false">Cerrar</v-btn>
            <v-btn
              color="success"
              :loading="saveLoader"
              :disabled="saveLoader"
    ...
    <!-- Precio Final  -->
    <v-dialog v-model="dialog.addWeight" max-width="800px">
      <v-form ref="weightForm">
        <v-card>
            ...
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex lg12 md12 sm12 xs12>
                  <v-select
                    label="Tipo de Elemento"
                    v-model="weightToInsert.product_type"
                ...
                >
                  <v-select
                    ...
                    v-model="weightToInsert.kilo_id"
                    @input="getKiloCost()"
                    :items="basic.kilos"
                    :rules="[v => !!v || 'Campo requerido']"
                    item-text="name"
                ...
                <v-flex
                  ...
                  v-if="weightToInsert.product_type == 1 || weightToInsert.product_type == 3"
                >
                  <v-text-field label="Valor por kilo" disabled v-model="weightToInsert.kilo_value"></v-text-field>
                </v-flex>
                ...
                <v-flex lg3 md3 sm8 xs8 v-if="weightToInsert.product_type != 3">
                  <v-text-field
                    ...
                    v-model="weightToInsert.actual_weight"
                  ></v-text-field>
                </v-flex>
                <v-flex lg12 md12 sm12 xs12 v-if=" weightToInsert.product_type == 3"></v-flex>
                <v-flex lg3 md3 sm12 xs12 v-if="weightToInsert.product_type == 3">
                  <v-text-field
                    label="Alto (cm)"
                    :rules="onlyNumbers"
                    v-model="weightToInsert.height"
                    @keyup="getTotalProductToInsert()"
                  ></v-text-field>
                </v-flex>
                <v-flex lg3 md3 sm12 xs12 v-if="weightToInsert.product_type == 3">
                ...
                <v-flex lg3 md3 sm12 xs12 v-if="weightToInsert.product_type == 3">
                  <v-text-field
                    label="Largo (cm)"
                    :rules="onlyNumbers"
                    v-model="weightToInsert.length"
                    @keyup="getTotalProductToInsert()"
                  ></v-text-field>
                </v-flex>
                <v-flex lg3 md3 sm12 xs12 v-if="weightToInsert.product_type == 3">
                  <h4 style="margin-top: 15px; margin-bottom: 0">
                    <small>Peso Kg: {{ volumetricForm }}</small>
                  </h4>
                </v-flex>
                <v-flex lg12 md12 sm12 xs12>
                  <h4 style="margin-top: 15px; margin-bottom: 0">
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="error" @click.native="dialog.addWeight = false">Cerrar</v-btn>
            <v-btn
              color="success"
              :loading="saveLoader"
              :disabled="saveLoader"
              @click="validateWeightForm()"
            >Agregar</v-btn>
          </v-card-actions>
        </v-card>
      </v-form>
  ...
  methods: {
    ...
    openAddProduct() {
      if (this.form.details.currency != "") {
        ...
        this.snackbar = {
          show: true,
          text: "Debe seleccionar la moneda de pago primero",
          color: "error",
    ...
    getKiloCost() {
      axios.get("kilo/" + this.weightToInsert.kilo_id).then(response => {
        this.weightToInsert.kilo_value = response.data.data.value;
      });
    },
    getKilos() {
    ...
    getCurrencies() {
      if (this.form.details.modality == 1) {
        var result = this.basic.countries.find(
          country => country.value == this.form.client.country
        );
      } else if (this.form.details.modality == 2) {
        var result = this.basic.countries.find(
          country => country.value == this.form.afiliates.country
        );
      }
      this.basic.money = result.money;
    },
    getOfficeTealca() {
      var result = this.basic.citys.filter(
        tealca => tealca.state == this.form.afiliates.city
      );
      this.basic.office_tealca = result;
    },
    selectOfficeTealca() {
    ...
    resetAddProductFields() {
      ...
      this.productToInsert.price_ensurance = "";
      this.$refs.productForm.reset();
    },
    addWeight() {
      this.form.weight = [];
      var rows = this.form.weight.length;
      var rows = rows + 1;
      var data = {
        package: rows,
        product_type: this.weightToInsert.product_type,
        height: this.weightToInsert.height,
        width: this.weightToInsert.width,
        length: this.weightToInsert.length,
        weight: this.weightToInsert.weight,
        actual_weight: this.weightToInsert.actual_weight,
        price: this.weightToInsert.price,
        kilo_id: this.weightToInsert.kilo_id,
        kilo_value: this.weightToInsert.kilo_value
      };
      this.cleanFields();
      this.form.weight.push(data);
      this.getTotal();
      this.dialog.addWeight = false;
    ...
    addProduct() {
      ...
      var data = {
        ...
        ensurance: this.productToInsert.ensurance,
    ...
    cleanFields() {
      ...
      this.productToInsert.ensurance = 0;
      this.productToInsert.price_ensurance = "";
      this.productToInsert.product_price = "";
      this.$refs.productForm.resetValidation();

      this.weightToInsert.product_type = "";
      this.weightToInsert.height = "";
      this.weightToInsert.width = "";
      this.weightToInsert.length = "";
      this.weightToInsert.weight = "";
      this.weightToInsert.actual_weight = "";
      this.weightToInsert.price = "";
      this.weightToInsert.kilo_id = "";
      this.weightToInsert.kilo_value = "";
      this.$refs.weightForm.resetValidation();
    },
    getTotalProductToInsert() {
      if (this.weightToInsert.product_type == 1) {
        // Por Kilo
        this.weightToInsert.weight = Math.ceil(
          this.weightToInsert.actual_weight
        );
        var kilo = this.weightToInsert.kilo_value,
          product_price = this.weightToInsert.product_price;
        var total = parseFloat(kilo) * parseFloat(this.weightToInsert.weight);
...
*/
        ...
        var height = this.weightToInsert.height,
          width = this.weightToInsert.width,
          length = this.weightToInsert.length,
          kilo = this.weightToInsert.kilo_value;

        var volumetric = 0;
        volumetric =
          parseFloat(width) * parseFloat(height) * parseFloat(length);
        volumetric = parseFloat(volumetric) / 5000;
        this.volumetricForm = volumetric.toFixed(2);
        this.weightToInsert.actual_weight = this.volumetricForm;
        this.weightToInsert.weight = Math.ceil(this.volumetricForm);
        var total = parseFloat(kilo) * parseFloat(this.weightToInsert.weight);
        total = total.toFixed(2);
    ...
    refreshToEnsurance() {
      if (this.productToInsert.ensurance == 1) {
        ...
        var total = (parseFloat(ensurance) * parseFloat(product_price)) / 100;
        total = total.toFixed(2);
        console.log(total);
        console.log("Informacion");
        this.productToInsert.price_ensurance = total;
      } else {
        this.productToInsert.price_ensurance = 0;
      }
    },
    getTotalProductToInsertEnsurance() {
      if (this.productToInsert.ensurance == 1) {
        var product_price = this.productToInsert.product_price,
          ensurance = this.productToInsert.ensurance_value;
        var total = (parseFloat(ensurance) * parseFloat(product_price)) / 100;
    ...
    setSubcategoryName() {
      if (this.productToInsert.subcategory != undefined) {
        axios
          .get("subcategory/" + this.productToInsert.subcategory)
          .then(response => {
    ...
    getSubcategories() {
      if (this.productToInsert.category !== undefined) {
        ...
        axios
          .get("category/" + this.productToInsert.category)
          .then(response => {
            this.productToInsert.ensurance_value =
              response.data.data.ensurance.value;
            this.productToInsert.category_name = response.data.data.name;
            //this.getTotalProductToInsert()
          });
        if (this.productToInsert.category != "") {
          axios
            .get("subcategory/by-category/" + this.productToInsert.category)
            .then(response => {
              if (response.data.status) {
    ...
    validateFindClient() {
      ...
      if (this.$refs.findClientForm.validate()) {
        axios
          .get("clients/by-number?key=" + this.search.client)
          .then(response => {
            if (response.data.status) {
    ...
    getTotal() {
      ...
      /*for (var i = 0; i < this.form.products.length; i++) {
        totalProduct += parseFloat(this.form.products[i].price);
      }*/
      var total = 0;
      for (var i = 0; i < this.form.weight.length; i++) {
        total += parseFloat(this.form.weight[i].price);
        this.form.details.product_type = this.form.weight[i].product_type;
        this.form.details.kilo_value = this.form.weight[i].kilo_value;
        this.form.details.kilo_id = this.form.weight[i].kilo_id;
        this.form.details.weight = this.form.weight[i].weight;
        this.form.details.actual_weight = this.form.weight[i].actual_weight;
        this.form.details.height = this.form.weight[i].height;
        this.form.details.width = this.form.weight[i].width;
        this.form.details.length = this.form.weight[i].length;
      }
      var taxNum = (parseFloat(total) * parseFloat(totalTax)) / 100;
      this.form.total =
        parseFloat(totalProduct) + parseFloat(total) + parseFloat(taxNum);
      this.form.total = parseFloat(this.form.total);
      this.form.total = this.form.total.toFixed(2);
    },
    getTotalEnsurance() {
      this.form.details.total_ensurance = 0;
      var total = 0;
      for (var i = 0; i < this.form.products.length; i++) {
        total += parseFloat(this.form.products[i].price_ensurance);
        console.log(this.form.products[i].price_ensurance);
      }
      this.form.details.total_ensurance = parseFloat(total);
      this.form.details.total_ensurance = this.form.details.total_ensurance.toFixed(
        2
      );
      console.log(this.form.details.total_ensurance);
    },
    resetClientFields() {
    ...
    blurValidateOrder() {
      if (this.$refs.addOrderForm.validate()) {
    ...
    updateClientIfSelected() {
      ...
      axios
        .put("clients/" + this.form.client.id, client)
        .then(response => {
    ...
    updateAfiliateIfSelected() {
      ...
      axios
        .put("afiliated/" + this.form.afiliates.id, afiliate)
        .then(response => {
          console.log(response.data.status);
        })
        .catch(error => {
    ...
    saveOrder() {
      var data = {
        ...
        kilo_value: this.form.details.kilo_value,
        kilo_id: this.form.details.kilo_id,
        weight: this.form.details.weight,
        actual_weight: this.form.details.actual_weight,
        height: this.form.details.height,
        width: this.form.details.width,
        length: this.form.details.length,
        taxes: this.taxesSelected,
        total: this.form.total,
        total_ensurance: this.form.details.total_ensurance,
        currency_ensurance: this.form.details.currency_ensurance,
        products: this.form.products
      };
      axios
        .post("ship-order", data)
        .then(response => {
          if (response.data.status) {
            this.snackbar = {
              show: true,
